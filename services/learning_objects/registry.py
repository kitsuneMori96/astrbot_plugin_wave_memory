...
_REGISTRY = (
    ...
    LearningObjectDescription(
        key="belief",
        source="ConsolidationService 摘要调用 BeliefEngine.extract_from_summary；BeliefEmergenceService 从 relationship_events 规则生成 pending 信念；未来 Agent review candidate",
        write_path="services.belief_engine.BeliefEngine.extract_from_summary -> db.add_belief(status='pending_legacy')；services.belief_emergence.BeliefEmergenceService.emerge_recent -> db.add_belief(status='pending', evidence_type='relationship_event')；engine.db.belief_repo.BeliefRepo.add_belief",
        ...
        dedup_rule="BeliefEngine 使用字符 Jaccard >0.6 强化已有信念；BeliefEmergence 对 pending+active 做字符集合相似度 >0.65 去重。",
        review_rule="BeliefEngine 摘要只写 pending_legacy，不直接 active；BeliefEmergence 写 pending；BeliefRepo 对身份污染直接 archived 且 strength=0.01。",
        ...
        risk="high",
        close_path="目前无专门 Belief_Settings.enabled；需通过 tag_llm_provider_id 缺失、Lifecycle_Settings.enable_consolidation=false 间接减少提取，但 main.py 仍会初始化 belief_emergence；memory_only 尚未默认阻止 belief 注入（第36项风险）。",
        audit_findings=(
            "重复写入风险：LLM 摘要信念和关系事件涌现信念使用不同去重阈值，可能产生语义重复的 pending/pending_legacy。",
