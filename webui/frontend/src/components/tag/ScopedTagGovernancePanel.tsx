...
export function ScopedTagGovernancePanel() {
  ...
  return <Card data-slot="scoped-tag-governance" className="border-primary/20">
    <CardHeader className="gap-3 border-b">
      ...
      <CardDescription>只操作当前 Bot + canonical session 的 scoped Tag。所有应用必须先预检。</CardDescription>
    ...
    <CardContent className="flex flex-col gap-5 p-4">
      {!scope ? <Alert><AlertTitle>等待正式 Scope</AlertTitle><AlertDescription>请选择真实 Bot 和 canonical session；不会从不完整的会话标识推断 Scope。</AlertDescription></Alert> : null}
      {scope ? <>
