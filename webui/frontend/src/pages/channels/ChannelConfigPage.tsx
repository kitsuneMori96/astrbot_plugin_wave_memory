...
import { RotateCcwIcon, SaveIcon, ShieldCheckIcon, WandSparklesIcon } from 'lucide-react'
...
function serializePatch(draft: ChannelConfigData): ChannelPatch {
  ...
  Object.entries(draft.channels ?? {}).forEach(([name, channel]) => {
    ...
    editableFields.forEach((field) => {
      if (field in channel) {
        channels[name][field] = channel[field] as never
      }
    })
    if (name === 'safety') {
...
export function ChannelConfigPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setDraft(payload.current ?? { channels: {}, recent_dedup_minutes: 30, trace_enabled: true })
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {validation ? <Badge variant={validationShape.ok ? 'secondary' : 'destructive'}>{validationShape.ok ? 'valid' : 'invalid'}</Badge> : null}
          </div>
        </CardContent>
      </Card>
