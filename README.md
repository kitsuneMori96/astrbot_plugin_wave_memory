...
[![Version](https://img.shields.io/badge/version-v5.2.0-blue.svg)](https://github.com/kitsuneMori96/astrbot_plugin_wave_memory/releases)
...
> [!WARNING]
> **本项目处于开发中（WIP），许多功能尚不可用。**
> 本仓库 fork 自上游 [vivy1024/astrbot_plugin_wave_memory](https://github.com/vivy1024/astrbot_plugin_wave_memory)——
> 从 fork 之时起直到现在，仍有相当一部分功能处于不可用或半可用状态：有的是上游遗留问题，
> 有的是重构过程中的过渡状态。下文的能力描述代表设计目标，**不代表全部已在当前提交中正常工作**。
> 使用前请以实际运行为准自行验证；遇到不可用的功能欢迎提 Issue 反馈。
...
- 📋 **L1 唤醒简报** — 被 @ 时注入群聊消息流水（零 LLM 成本），解决冷启动问题
- 🏗️ **三层成本结构** — L0 原文+FTS（零成本）→ L1 简报（零成本）→ 抽取层（高价值信号才付费处理）

### Recent Releases

| 版本 | 日期 | 重点 |
|------|------|------|
| **v5.2.0** | 2026-09-18 | 架构重构：知识层归人（跨群共享 user_id）、群聊/私聊分治、三层成本结构（L0 原文+FTS / L1 简报 / 抽取层高价值信号）；L1 唤醒简报（BriefingChannel）；水源重生（75 facts G1:34 G2:41 + 292 user_profiles）；consolidation prompt 重写（G1-G4 四分组 + temporal_tier） |
